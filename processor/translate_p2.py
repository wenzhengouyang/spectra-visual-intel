#!/usr/bin/env python3
"""Localize P2 news briefs for the Chinese SPECTRA reading interface.
...
def fields_needing_translation(brief: dict[str, Any]) -> list[str]:
    return [field for field in ("headline", "dek") if not has_chinese(brief.get(field))]
...
def validate_translation(source: str, translated: str, field: str) -> None:
    if not has_chinese(translated):
        raise ValueError(f"{field} translation does not contain Chinese")
    if not numbers_match(source, translated):
        missing = raw_number_tokens(source) - raw_number_tokens(translated)
        raise ValueError(
            f"{field} translation lost or changed numbers/units: {sorted(missing)}; "
...
def apply_translation(brief: dict[str, Any], translated: dict[str, Any]) -> None:
    ...
    for field in requested:
        ...
        brief[field] = target
    if requested:
        brief["localization_status"] = "machine_translated_review_pending"
...
def main() -> None:
    ...
    localized_total = sum(
        brief.get("localization_status") == "machine_translated_review_pending"
    ...
    write_json(output_path, issue)
    write_json(checkpoint_path, {"status": "completed", "translated": len(targets), "total": len(targets), "batches": batches})
    if args.static:
