...
def publication_quality_errors(
    ...
    if stories and core_event_count < minimum_core_events:
    ...
    for item in [*stories, *briefs]:
        ...
        if item.get("story_id"):
            ...
            if kind == "official" and not str(cover.get("source_url") or "").startswith("https://"):
                errors.append(f"{item_id}.official_cover_missing_source")
            if kind in {"editorial_diagram", "generated"}:
