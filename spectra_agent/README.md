...
异常统一写入运行目录的 `reliability.json`，字段固定为发生阶段、影响范围、错误类别、
错误码、原始原因、是否可重试、当前/最大次数、处理动作和下一步。网络、模型及发布类
故障最多自动恢复两次；配置、授权、数据损坏和质量闸门不盲目重试。辅助阶段失败记录为
`degraded`，不会把已成功生成的内容改成失败；超过上限后只允许局部降级或转人工。
...
频控或空结果时停止。增量采集遇到 WeRSS 失败或降级时不会再立即执行完整 7 日补采；
HTTP 429 会遵循 `Retry-After`（否则指数退避），并对该来源执行 24 小时冷却，冷却状态
和跳过原因写入运行日志与 `run.json`。
...
评测默认只读流水线产物、只写上述报告。主流程在发布物校验之后、标记 `completed` 之前按 `run_evaluation.enabled` 调用；当前 `block_completion_on_failure=true`，结构错误、来源健康度、重复处理、P1/P2 中文完整度或图片质量不达标会阻止 `completed` 和发布。Writer 降级率与全事实池抽检失败率属于自动化成熟度预警：它们会阻止扩大自动放行范围，但在最终稿已经通过人工事实审核、发布物校验和逐篇语义核验时，不单独否定本期发布。深读不设最低字数，要求在事实保真、证据支持和不重复的前提下尽量覆盖有效信息。只有事实审核、Writer、页面校验全部结束且发布前校验通过的不同真实窗口才标记为 `cohort_eligible` 并计入滚动判断；在 `waiting_for_review` 阶段做的手动回放不会冒充完整轮次。滚动建议中的 `allow_expand` 只供人判断，评测不会修改 `p1-review.json`、claims、policy 或发布权限。模型、采集策略或窗口配置变化会改变策略指纹，使既有“稳定”结论失效并重新进入观察。
